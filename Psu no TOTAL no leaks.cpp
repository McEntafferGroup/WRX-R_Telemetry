...
unsigned int __stdcall mythread(void*)
{
	...
	*/
	...
	// data_location   - the position in the DATA1 list into which the next pixel value will be placed  //
	//=================================================================================================//
						...
						else
						{
							...
							if (word == e1234)
							{
								...
								if ((row == num_row - 1) && (graphing))
								{
									...
									if (!half)
									{
										PyObject_CallFunctionObjArgs(sub_plot, DATA2, DATA1, TOTAL, FRAME_NUMBER, first_or_second, lt, ts0, ts1, NULL);
									}
									else
									{
										PyObject_CallFunctionObjArgs(sub_plot, DATA1, DATA2, TOTAL, FRAME_NUMBER, first_or_second, lt, ts0, ts1, NULL);
									}
									
									/*the previous frame's data is then given to the DATA2 variable for proper use in the next frame*/
									//DATA2 = PyObject_CallFunctionObjArgs(CREATE, DATA1, DATA2, NULL);
									//DATA2 = DATA1;
									//DATA2 = Py_BuildValue("N", DATA1);
									//Py_DECREF(DATA1);
									//PyMem_Free(DATA1);
									//Py_DECREF(TOTAL);
									//PyRun_SimpleString("gc.collect()");
