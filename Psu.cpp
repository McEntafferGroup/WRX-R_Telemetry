...
unsigned int __stdcall mythread(void*)
{
	...
	*/
	...
	// make_list - creates the empty Python list of the correct size for DATA1 storage                  //
	//=================================================================================================//
	...
	/*Python variable used to store the pixel values before being added to the DATA1 list*/
	//PyObject* temp_dat = Py_BuildValue("i", 0);

	//make a huge list in python so values can be placed out of order
	//PyObject* det_size = PyLong_FromLong(num_row*num_column);

	/*The next section is a load of Python variables used within the plotting function*/
	//=================================================================================================//
	// DATA1           - the current frame's data/pixel values (alternating)                           //
	// DATA2           - the last frame's data/pixel values    (alternating)                           //
	...
	PyObject* ts0p = PyLong_FromLong(1);
	PyObject* ts0 = PyLong_FromLong(1);
	PyObject* ts1p = PyLong_FromLong(1);
	PyObject* ts1 = PyLong_FromLong(1);
	...
	const int reset_number = 75; //subtract one so it removes the following number
	...
	while (lThreadRun)
	{
		...
		//PyObject* DATA1 = PyObject_CallFunctionObjArgs(make_list, det_size, NULL);
		//process data
				...
				for (int half = 0; half<2; half++)
				{
					...
					case S_TEMP_02:
					{
						if (check == t02)
						{
							...
							{
								ts0p = PyLong_FromLong((temp1 << 12) | (word & 0x0FFF));
							}
							/*advance the state*/
							STATE = S_TEMP_11;
						}
					...
					case S_TEMP_12:
					{
						if (check == t12)
						{
							...
							{
								ts1p = PyLong_FromLong((temp2 << 12) | (word & 0x0FFF));
							}
						...
						else
						{
							...
							if (word == e1234)
							{
								...
								if ((row == num_row - 1) && (graphing))
								{
									...
									if (frame_counter % 2)
									{
										PyObject_CallFunctionObjArgs(sub_plot, DATA2, DATA1, FRAME_NUMBER, first_or_second, lt, ts0p, ts0, ts1p, ts1, NULL);
									}
									else
									{
										PyObject_CallFunctionObjArgs(sub_plot, DATA1, DATA2, FRAME_NUMBER, first_or_second, lt, ts0p, ts0, ts1p, ts1, NULL);
									}
								}
							}
							else
