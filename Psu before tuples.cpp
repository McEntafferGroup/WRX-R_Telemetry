...
unsigned int __stdcall mythread(void*)
{
	...
	*/
	...
	PyObject* PREVIOUS = PyObject_CallFunctionObjArgs(CREATE, DATA, NULL);
	PyObject* TOTAL = PyObject_CallFunctionObjArgs(make_list, det_size, NULL);
						...
						else
						{
							...
							if (word == e1234)
							{
								...
								if ((row == num_row - 1) && (graphing))
								{
									...
									//===================================================================================================//
									TOTAL = PyObject_CallFunctionObjArgs(sub_plot, DATA, DATA, TOTAL, FRAME_NUMBER, first_or_second, lt, ts0, ts1, NULL);
									/*the previous frame's data is then given to the PREVIOUS variable for proper use in the next frame*/
									//PREVIOUS = PyObject_CallFunctionObjArgs(CREATE, DATA, NULL);
									//PREVIOUS = DATA;
									Py_DECREF(DATA);
									//PyMem_Free(DATA);
									Py_DECREF(lt);
